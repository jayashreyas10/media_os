...
- **Deploy Pipeline**: [`scripts/prepare-db.js`](file:///c:/Users/ensen/Desktop/MT/agent/scripts/prepare-db.js) $\rightarrow$ `npx prisma db push` $\rightarrow$ `npm start`.
- **Automated Database Switching**: If Railway PostgreSQL is attached, `prepare-db.js` automatically configures Prisma for PostgreSQL. If no database is attached, it defaults to SQLite.
...
3. Select your `agent` / `mediaos` repository.

### Step 3: Add PostgreSQL Database (Recommended for Production)
...
| `SESSION_SECRET` | *(64-hex string)* | Used to sign operator authentication sessions. |
...
| `AI_PROVIDER_DEFAULT` | `mock` *(or `gemini`, `openai`, `anthropic`)* | Active AI provider mode. |
| `PORT` | `3000` | Optional (Railway assigns port automatically via `$PORT`). |

> **Quick Key Generator**: Run this command in your local terminal to generate your secrets:
> ```bash
